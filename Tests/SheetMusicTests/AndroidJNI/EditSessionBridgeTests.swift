    ...
    /// Kotlin hop. Handles are created through the shipped `nativeLoadScore`
    /// so the table's lifetime rules are exercised too.
    ...
    struct EditSessionBridgeTests {
        ...
        /// `EditReplayScript`'s doc comment) — there is no rest anywhere in the fixture,
        /// so `.inputNote` (which requires an existing rest at its target) would be refused
        /// at any slot. `.setChordDuration` on an already-existing chord exercises the same
        /// round-trip (decode -> apply -> publish) without that precondition.
