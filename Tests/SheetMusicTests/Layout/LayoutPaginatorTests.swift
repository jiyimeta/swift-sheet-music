...
import Testing
    ...
    /// `CGPoint` and `CGSize` (see `Sources/SheetMusicLayout/Fonts/CGTypes+Android.swift`), so
    /// anchor explicitly to SheetMusicLayout's own definitions instead of leaving them ambiguous.
    ///
