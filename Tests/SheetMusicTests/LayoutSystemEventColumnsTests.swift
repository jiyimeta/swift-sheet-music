    ...
    // Swift Testing's `#expect` macro rewrites the comparison below into an
    // operator-overload chain to build its failure-message expression tree.
    // On Apple that overload resolution needs `CGFloat`'s `FloatingPoint`
    // conformance in scope explicitly — without this import, `.max() ?? 0`
    // type-checks as still-optional and the `#expect` line fails to compile.
    // The file itself never spells out `CGFloat` (see the `CGTypes+Android`
    // note in other files in this target for why that would need an anchor).
    import CoreGraphics
#endif
