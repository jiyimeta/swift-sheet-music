...
struct LayoutCacheBenchmark {
    ...
    func benchmark() throws {
        ...
        print(String(format: "cold (populate cache): %7.1f ms",
                     coldCacheMs))
        ...
        print(String(format: "warm (all cached):     %7.1f ms",
                     warmMs))
        print("warm hits: width=\(cache.widthHits) "
              + "placement=\(cache.placementHits)")
        print(String(format: "  speedup vs cold:    %.0f%% "
                     + "(saved %.1f ms)",
                     (1 - warmMs / coldMs) * 100,
                     coldMs - warmMs))
        ...
        if !staves.isEmpty,
           let editIdx = pickEditableMeasure(staff: staves[0]) {
            ...
            print(String(format: "edit (1 measure):      %7.1f ms",
                         editMs))
    ...
    private func bumpFirstChord(in measure: Measure) -> Measure {
        ...
        for vi in voices.indices {
            ...
            for ei in elements.indices {
                if case .chord(var c) = elements[ei], !c.notes.isEmpty {
                    let n = c.notes[0]
                    c.notes[0] = Note(
