...
struct SheetMusicEngineTests {
    ...
    func versionMatchesRelease() {
        ...
        // release process that touches it. The `-w9` suffix marks a build published only to a
        // local `~/.m2`, so it stamps differently from the eventual tagged one — see the
        // constant's own doc for why that difference is load-bearing.
        #expect(SheetMusicEngine.version == "1.15.0-w9")
    }
    ...
    func stampMatchesTheWrittenOutValue() {
        ...
        // wrong hash agree with itself. 1.13.1 was 7339729597660573583 and 1.15.0 will be
        // 3598528033961937950, so the three are visibly distinct.
        #expect(SheetMusicEngine.versionStamp == 3_188_487_432_499_784_217)
    }
}
