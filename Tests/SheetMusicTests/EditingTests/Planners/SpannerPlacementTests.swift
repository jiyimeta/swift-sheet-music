...
struct SpannerPlacementTests {
    ...
    func storageForms() {
        ...
        for kind in [Spanner.Kind.hairpin, .pedal, .ottava, .textLine, .trill, .vibrato, .palmMute, .letRing] {
    ...
    func lineSpanner() throws {
        ...
        _ = try SpannerPlacement.add(Self.hairpin(), over: range, in: score).apply(to: &score)
    ...
    func slur() throws {
        ...
        _ = try SpannerPlacement.add(Spanner(kind: .slur, rawType: "Slur"), over: range, in: score)
            .apply(to: &score)
    ...
    func rangeNormalization() throws {
        ...
        _ = try SpannerPlacement.add(Self.hairpin(), over: reversed, in: score).apply(to: &score)
        guard case .spanner = Self.elements(score, 0, 0)[1] else { Issue.record("expected the spanner at 1"); return }
        ...
        _ = try SpannerPlacement.add(Self.hairpin(), over: across, in: crossStaff).apply(to: &crossStaff)
    ...
    func undoIsExact() throws {
        for template in [Self.hairpin(), Spanner(kind: .slur, rawType: "Slur")] {
            ...
            let before = score
            let range = VoiceElementRange(start: Self.slot(Self.flute, 2, 0), end: Self.slot(Self.flute, 2, 1))
            let inverse = try SpannerPlacement.add(template, over: range, in: score).apply(to: &score)
            #expect(score != before)
    ...
    func removeRoundTrips() throws {
        for template in [Self.hairpin(), Spanner(kind: .slur, rawType: "Slur")] {
            ...
            let plain = score
            let range = VoiceElementRange(start: Self.slot(Self.flute, 2, 0), end: Self.slot(Self.flute, 2, 1))
            _ = try SpannerPlacement.add(template, over: range, in: score).apply(to: &score)
            let written = score
            let inverse = try SpannerPlacement.remove(template.kind, at: Self.slot(Self.flute, 2, 0), in: score)
                .apply(to: &score)
            #expect(score == plain)
            _ = try inverse.apply(to: &score)
    ...
    func duplicate() throws {
        ...
        _ = try SpannerPlacement.add(Spanner(kind: .pedal, rawType: "Pedal"), over: shifted, in: score)
            .apply(to: &score)
        let after = score
        ...
        let error = #expect(throws: SheetMusicError.self) {
            _ = try SpannerPlacement.add(Self.hairpin(), over: again, in: score)
        }
    ...
    func refusals() throws {
        ...
        let lonely = #expect(throws: SheetMusicError.self) {
            _ = try SpannerPlacement.add(Spanner(kind: .slur, rawType: "Slur"), over: single, in: score)
        }
        ...
        let goneAway = #expect(throws: SheetMusicError.self) {
            _ = try SpannerPlacement.add(Self.hairpin(), over: missing, in: score)
        }
        ...
        let nothingThere = #expect(throws: SheetMusicError.self) {
            _ = try SpannerPlacement.remove(.slur, at: Self.slot(Self.flute, 2, 0), in: score)
        }
        ...
        let notAnElement = #expect(throws: SheetMusicError.self) {
            _ = try SpannerPlacement.remove(.hairpin, at: Self.slot(Self.flute, 0, 0), in: score)
        }
        ...
        _ = try SpannerPlacement.add(Self.hairpin(), over: hairpinRange, in: withHairpin).apply(to: &withHairpin)
        let wrongKind = #expect(throws: SheetMusicError.self) {
            _ = try SpannerPlacement.remove(.pedal, at: Self.slot(Self.flute, 2, 0), in: withHairpin)
        }
