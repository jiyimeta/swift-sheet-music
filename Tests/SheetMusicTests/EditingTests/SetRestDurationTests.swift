...
struct SetRestDurationTests {
    ...
    func shortenQuarterToEighth() throws {
        ...
        let els = first(score)
        guard case let .chord(r0) = els[0], r0.notes.isEmpty else {
    ...
    func shortenWholeToEighthAligned() throws {
        ...
        let durations = els.compactMap { el -> NoteDuration? in
            if case let .chord(r) = el, r.notes.isEmpty { return r.duration }
            return nil
        }
    ...
    func lengthenIntoChord() throws {
        ...
        #expect(r0.duration == .quarter)
        guard case let .chord(c1) = els[1] else {
        ...
        guard case let .chord(c2) = els[2] else {
