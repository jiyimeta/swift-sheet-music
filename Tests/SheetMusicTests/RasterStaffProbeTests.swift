    ...
    struct RasterStaffProbeHarness {
        ...
        func printLines(_ tag: String, _ pairs: [(Double, Double)], minValue: Double = 40) {
            let wide = pairs.filter { $0.1 > minValue }.sorted { $0.0 > $1.0 }
            let text = wide.map {
        ...
        @Test func compareDetectedStaves() throws {
            ...
            for dir in try OMRHarnessDirectoryWalk.renderDirectories(root: root).prefix(3) {
                for name in try OMRHarnessDirectoryWalk.labelFiles(in: dir).prefix(1) {
                    try comparePage(dir: dir, labelFile: name)
                }
            }
        }

        /// One page, split out of the loop above only to stay under the
        /// 60-line function-body cap.
        func comparePage(dir: String, labelFile name: String) throws {
            ...
            if ProcessInfo.processInfo.environment["OMR_STAFF_PROBE_LINES"] == "1" {
                ...
                let tv = replay.walked.paths.filter {
                    $0.pageIndex == idx && $0.kind == .vertical
                }
                printLines("oracleV", tv.map {
                    (Double($0.rect.midX), Double($0.rect.height))
                }, minValue: 10)
                printLines(
                    "rasterV",
                    analysis.paths.filter { $0.kind == .vertical }
                        .map { (Double($0.rect.midX), Double($0.rect.height)) },
                    minValue: 10,
                )
            }
        }
