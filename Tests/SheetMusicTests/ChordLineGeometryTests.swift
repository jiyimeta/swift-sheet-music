...
import Testing
    ...
    /// On Android, Foundation's own `CGFloat`/`CGPoint` are also visible alongside
    /// SheetMusicLayout's (see `Sources/SheetMusicLayout/Fonts/CGTypes+Android.swift`'s
    /// warning), and mixing `import Foundation` with `@testable import SheetMusicLayout`
    /// leaves the bare names ambiguous. Anchor explicitly to SheetMusicLayout's own
    /// definitions, since `ChordLineGeometry.defaultPath` below is what actually consumes
    /// them.
    ///
...
struct ChordLineGeometryTests {
    ...
    /// `private` type.
    private static let sp: CGFloat = 10
