...
import io.github.kiichiio.sheetmusic.audio.model.ClefAnchor
...
/**
 * Round-trip tests for all encoders in [Encoders.kt].
 * Encode then decode should equal the original value.
 */
class EncoderTest {
    ...
    @Test fun noteIDCodec_roundTrip() {
        ...
        val decoded = NoteIDDecoder.decode(encoded)
    ...
    @Test fun scoreItemIDCodec_note_roundTrip() {
        ...
        val decoded = ScoreItemIDDecoder.decode(encoded)
    ...
    @Test fun scoreItemIDCodec_encodeArray_roundTrip() {
        ...
        val encoded = ScoreItemIDCodec.encodeArray(ids)
        val decoded = ScoreItemIDDecoder.decodeArray(encoded)
        assertEquals(ids, decoded)
    }

    @Test fun scoreItemIDCodec_emptyArray_roundTrip() {
        val encoded = ScoreItemIDCodec.encodeArray(emptyList())
        val decoded = ScoreItemIDDecoder.decodeArray(encoded)
    ...
    @Test fun scoreCursorCodec_item_roundTrip() {
        ...
        val decoded = ScoreCursorDecoder.decode(encoded)
    ...
    @Test fun metronomeBeatCodec_roundTrip() {
        ...
        val encoded = MetronomeBeatCodec.encodeArray(beats)
        val decoded = MetronomeBeatDecoder.decodeArray(encoded)
        assertEquals(beats, decoded)
    }

    @Test fun metronomeBeatCodec_emptyArray_roundTrip() {
        val encoded = MetronomeBeatCodec.encodeArray(emptyList())
        val decoded = MetronomeBeatDecoder.decodeArray(encoded)
    ...
    @Test fun staffParamsCodec_roundTrip() {
        ...
        val encoded = StaffParamsCodec.encodeArray(params)
        val decoded = StaffParamsDecoder.decodeArray(encoded)
