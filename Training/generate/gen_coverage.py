...
def coverage_sources(seed: int) -> list[tuple[str, str]]:
    ...
    # 4. Time signatures: digits 0-9 via changing meters (10/4 covers 0 and 1).
    ts_measures = []
    ...
    for _ in range(4):
        ...
        tie_measures.append("\n".join(
            ...
            + [chord(p, 14, duration="half"),
               chord(p, 14, duration="half", tie="start", tie_frac="2/4")]))
        tie_measures.append("\n".join(
            [chord(p, 14, duration="half", tie="stop", tie_frac="2/4"),
             chord(p, 14, duration="quarter", tie="start"),
             chord(p, 14, duration="quarter", tie="stop")]))
