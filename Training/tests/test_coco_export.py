...
def test_image_pixel_size_prefers_actual_png_on_disk_over_recomputation(tmp_path):
    ...
    truth and must win over any recomputed page_size_px value -- even a
    correct recomputation, because rasterize.py's own contract is "if a
    rendered image already exists, read its actual .size -- never
    recompute it" (e.g. a page that was itself produced by a geometric
    degradation stage genuinely has different pixel dimensions than its
    page-box points would predict)."""
    render = tmp_path / "r0"
    render.mkdir()
    (render / "render.json").write_text(json.dumps({"schema": 1, "dpi": 300}))
    (render / "page_0.labels.json").write_text(json.dumps({
