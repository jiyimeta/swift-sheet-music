...
public struct ScoreHitTester: Sendable {
    ...
    private func hitNote(
        ...
        for el in measure.elements {
            guard case let .chord(notes, _, stem, _, _, _, _, _, _, _) = el
            else { continue }
            for n in notes {
    ...
    private func beamedChordNotes(
        ...
        for el in measure.elements {
            guard case let .chord(
                notes, _, _, stemOrigin, _, _, isBeamed, _, _, _,
            ) = el,
                isBeamed,
    ...
    private func hitFlag(
        ...
        for el in measure.elements {
            guard case let .chord(
                notes, dur, stem, _, _, _, isBeamed, _, _, _,
            ) = el,
                !isBeamed,
                Self.hasFlag(dur),
    ...
    private func hitStem(
        ...
        for el in measure.elements {
            guard case let .chord(
                notes, _, stem, stemOrigin, _, _, isBeamed, _, _, _,
            ) = el,
                let noteX = notes.first?.origin.x
            else { continue }
            let ys = notes.map(\.origin.y)
