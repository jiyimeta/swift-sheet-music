...
///
/// On Android (Phase 1), `.mxl` is not yet supported. Use uncompressed
/// `.musicxml` files instead. A future phase will lift this restriction
...
enum MXLReader {
    ...
    static func extractRootScore(mxlData: Data) throws -> Data {
            ...
            throw SheetMusicError.unsupportedFeature(
                name: "MXL (compressed MusicXML)",
                location: "MXLReader: ZIP containers are not supported on Android in Phase 1",
            )
        #else
