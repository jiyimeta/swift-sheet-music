...
extension LiveChannelPlan {
    ...
    public struct StripLabels: Sendable, Equatable {
        ...
        /// The instrument driving the strip, unqualified by the part. Always reported, even when `displayName`
        /// suppresses it: what a host chooses to draw is not this type's business.
    ...
    /// - an instrument whose name equals the part's is suppressed. The primary strip's instrument name equals the
    ///   part label by construction, so without this a two-instrument part reads "Piano (Piano)" on its first row.
