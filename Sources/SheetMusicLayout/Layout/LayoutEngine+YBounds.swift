...
extension LayoutEngine {
    ...
    static func shiftLyricTextY(
        ...
        switch element {
        case let .textMark(.lyrics(color, verse, anchor, placement, properties), text, p):
            return .textMark(
                kind: .lyrics(
                    color: color, verse: verse, anchor: anchor, placement: placement, properties: properties,
                ),
                text: text,
                origin: bump(p),
            )
