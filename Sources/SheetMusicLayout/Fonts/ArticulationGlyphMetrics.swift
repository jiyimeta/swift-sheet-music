...
/// typographic frame centre, NOT the visible ink. MuseScore instead centres
...
/// centre, so a glyph anchored naively renders ~0.17 sp too far from the note.
///
/// The layout queries this offset and shifts `origin.y` inward so the rendered
/// ink centre lands exactly on the reference Y, matching MuseScore.
public enum ArticulationGlyphMetrics {
    /// Screen-Y-down distance (at `fontSize = 4`, i.e. 1 sp = 1 unit) from the
    /// `.center`-anchored `origin.y` to the glyph's INK centre. Callers scale
    ...
    public static func inkCenterOffset(codepoint: UInt16) -> CGFloat {
        ...
        // `.center` puts the typographic centre at origin.y; the baseline sits
        ...
        // bbox is CT (Y-up) about the baseline; ink centre below origin.y is
        // baselineFromCenter (baseline) minus the Y-up ink centre.
