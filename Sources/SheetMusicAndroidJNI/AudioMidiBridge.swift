...
// so host-platform tests can call the helpers directly. The Kotlin-facing
// entry points (now generated by swift-java JExtractSwiftPlugin) live in
// this file and its `+Render` / `+Timeline` siblings, defined as plain
// `public func` declarations rather than `@_cdecl` thunks.
...
// MARK: - JNI byte-array helpers (Android only)

#if os(Android)
    import CJNI

    // Shared by the remaining `@_cdecl` thunks in this target's
    // SheetMusic-side bridges (`CursorBridge`, `LoopHighlightBridge`,
    // `JNISymbols`). The Audio-side bridges no longer use these; the
    // SheetMusic side will be migrated to swift-java in a follow-up task.
