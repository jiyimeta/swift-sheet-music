...
3. **単独で追加できるMISSING** — ~~`ORNAMENT`~~（2026-09-04完了、§4.6の追記）、
   ...
   `StringData`でもう1段軽いところが分かった: **`<Instrument>`配下のようにscore構造の外側に
   ぶら下がる要素は、`VoiceElement`にもfingerprintにも触れずに終わる**。`Instrument`は
   `ScoreFingerprintHasher`が歩いていないので、occupants方式のhasher追加すら要らなかった。
   3段ある——instrument / part配下、note・chord添付、voice stream。
