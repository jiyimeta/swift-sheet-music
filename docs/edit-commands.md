...
| `DuplicateRange` | `R` (repeat selection: copies the range to just after itself; appends bars when the copy runs past the end of the score; carries the range's own clefs, breaths, slurs and lines and its staff annotations — dynamics, fermatas, chord symbols, expression, sticking, capos, string tunings, figured bass, symbols and fret diagrams. Landing, it clears the destination's staff annotations under it, except a clef, breath or chord symbol at the same tick: a copied one supersedes, a copied one arrived replaces the destination one. The rules MuseScore's own paste applies) | — |
| `PasteRange` | ⌘V (a copied range: writes a clipboard payload's material — a small, self-contained `Score` of its own, so it may have come from another score or another window — into the destination through `DuplicateRange`'s own write pass, so every rule a duplicate obeys a paste obeys too) | sugar |
...
more commands: `SetNoteSmall` (80), `SetNotePlay` (81), `SetTextOffset`
(82) and `SetTextAutoplace` (83). Same convention:
...
| `SetTextOffset` (82) / `SetTextAutoplace` (83) | Both take a `ScoreTextID` — lyric, staff/system text, harmony, rehearsal mark — not a note, chord or spanner, because those four kinds are the only carriers whose `offset` and `autoplace` the layout reads. `offset` is read in one place, `LayoutEngine.placedTextOrigin`, whose callers cover exactly those four kinds across five call sites: the fifth places the text-entry cursor from the same authored offset, one more reader of the same four kinds rather than a fifth kind. `autoplace` is read at four `TextPlacementMetadata(autoplace:)` sites in `LayoutEngine+Placement.swift`, one per kind, plus two more construction sites for the lyric kind's own continuation rows (`LayoutEngine+Lyrics.swift`, `LayoutEngine+LyricHyphenSpans.swift`) — more sites, still the same four kinds, not a repo-wide claim of "nowhere else". `ElementProperties` carries both fields on every engravable — a note, a chord, a dynamic and a spanner all store and round-trip them — but there they are storing-but-inert, the same pattern `SetElementColor` (76) already found for `Chord.elementProperties.color`. |
